# STM32 Mini-Project Milestones
---
A collection of small, focused projects to mark key checkpoints in my learning journey as an **embedded software engineer**.
## Hardware ⚙️
---
- **Board**: [STM32F407G-DISC1](https://www.st.com/en/evaluation-tools/stm32f4discovery.html) 
- **Microcontroller**: STM32F407VG (ARM Cortex-M4)
## Development Environment 🧰
---
- **IDE**: STM32CubeIDE
- **Compiler**: ARM GCC
- **Language:** C
## References 📚
---
- [User Manual](https://www.st.com/resource/en/user_manual/um1472-discovery-kit-with-stm32f407vg-mcu-stmicroelectronics.pdf)
- [Reference Manual](https://www.st.com/resource/en/reference_manual/dm00031020-stm32f405-415-stm32f407-417-stm32f427-437-and-stm32f429-439-advanced-arm-based-32-bit-mcus-stmicroelectronics.pdf)
- [Datasheet](https://www.st.com/resource/en/datasheet/dm00037051.pdf)
- [ARM Cortex-M4 Generic User Guide](https://documentation-service.arm.com/static/5f2ac4ab60a93e65927bbdbf?token=)
## Goals
---
- [ ] Learn the fundamentals of embedded software development
- [ ] Learn common hardware interface protocols (USART, SPI, I2C, etc.)
- [ ]  Learn real-time operating systems (RTOS)
- [ ] Finish a project I would use in real-life